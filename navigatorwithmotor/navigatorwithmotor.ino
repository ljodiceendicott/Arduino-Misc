...
class MotorController{
  class Motor{
    ...
    public:
      ...
      void asyncRampSpeed(int speed, int rate){
        this->phase = RAMP_PHASE;
        this->rate = rate;
        this->new_speed = speed;
      }
      void rampSpeed(int speed, int rate){
        if(speed < this->speed){
          for(int i = this->speed - rate; i > 0; i-=rate){
            setSpeed(i);
          }
        }
        else if(speed > this->speed){
          for(int i = this->speed; i < speed; i+=rate){
            setSpeed(i);
          }
        }
      }
...
int waypointButton = 3;
int waypointLight = 7;
bool found = false;

int waypointLastidx = 0; 
int maxSize = 10;
float waypointLong[20];
float waypointLat[20];
...
const int trigcenter = 45;
const int echocenter = 47;
...
// int waypointlight = 7;
  ...
  if (millis() > 5000 && gps.charsProcessed() < 10)
    ...
    if(waypointLastidx == maxSize){
      Serial.print("Sorry Maximum waypoints reached");
    }
    else{
      waypointLong[waypointLastidx] = gps.location.lng();
      waypointLat[waypointLastidx] = gps.location.lat();
      waypointLastidx = waypointLastidx + 1;
    }
  }
  Serial.println("Current Waypoints:");
  for(int i = 0; i < waypointLastidx; i++){
 ...
 if(waypointLastidx>1){ 
  unsigned long distanceKmAwayFromWaypointOne =
      (unsigned long)TinyGPSPlus::distanceBetween(
        gps.location.lat(),
        gps.location.lng(),
        waypointLat[0], 
        waypointLong[0]) / 1000;
    if(distanceKmAwayFromWaypointOne <= 0.0009144){//0.0009144 is 3 feet
      //Waypoint is found / Navigator is within 3 feet or less
      found = true;    
    }
    else{
      found = false;
 ...
 if(found){
   digitalWrite(waypointLight, HIGH);
 }
}
...
void ultrasonic(){
  ...
  if(leftdist<20){//object in the way
 ...
 if(centerdist<20){//object in the way
 ...
 if(rightdist<20){//object in the way
