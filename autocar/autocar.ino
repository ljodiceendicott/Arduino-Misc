...
public:
    ...
    void autoCar(long currlon, long currlat, int leftdis, int centdis, int rightdis, Waypoint[] waypoints, int waypoint_num, int yaw, int courseToWaypoint)
    {
        if (Math.abs(currlon - waypoints[waypoint_num].lng) > 5 && Math.abs(currlat - waypoints[waypoint_num].lat) > 5)
...
void loop()
{
    ...
    {
        mc.autoCar(gps.location.lng(), gps.location.lng(), left_us.getDist(), center_us.getDist(), right_us.getDist(), waypoints, yaw, courseToWaypoint);
    }
